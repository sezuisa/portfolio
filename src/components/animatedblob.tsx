...
export default function AnimatedBlob({
  ...
  const id = useId();
  const durationPerSegment = 6; // seconds
  const totalSegments = blobPaths.length;
  const totalDuration = durationPerSegment * totalSegments;

  const values = [...blobPaths, blobPaths[0]].join(";");
  const keyTimes = [...Array(totalSegments + 1).keys()]
    .map((i) => (i / totalSegments).toFixed(4))
    .join(";");
  const keySplines = Array(totalSegments).fill("0.42 0 0.58 1").join(";");
